        ...
        for (int x = xMin; x < xMax; ++x) {
            if (insideTriangle(x + 0.5, y + 0.5, t.v)) {
                ...
                if (depth_buf[index] >= zp) {
                    ...
                    auto interpolated_color = (t.color[0] / v[0].w() * alpha + t.color[1] / v[1].w() * beta +
                                               t.color[2] / v[2].w() * gamma) * Z;
                    auto interpolated_normal = (t.normal[0] / v[0].w() * alpha + t.normal[1] / v[1].w() * beta +
                                                t.normal[2] / v[2].w() * gamma) * Z;
                    auto interpolated_tex_coords =
                            (t.tex_coords[0] / v[0].w() * alpha + t.tex_coords[1] / v[1].w() * beta +
                             t.tex_coords[2] / v[2].w() * gamma) * Z;
                    auto interpolated_shading_coords = (view_pos[0] / v[0].w() * alpha + view_pos[1] / v[1].w() * beta +
                                                       view_pos[2] / v[2].w() * gamma) * Z;;
                    fragment_shader_payload payload(interpolated_color, interpolated_normal.normalized(),
                                                    interpolated_tex_coords, texture ? &*texture : nullptr);
